...
export default function DeleteModal({ isOpen, onClose, carId }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-200">
      <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl dark:bg-slate-900 border border-slate-100 dark:border-slate-800 scale-100 animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
          ...
          <div>
            <h3 className="text-lg font-black text-slate-900 dark:text-white">Delete Car Listing</h3>
            <p className="text-xs text-slate-400 font-medium">This action is permanent and cannot be undone.</p>
          </div>
        </div>
        ...
        <div className="py-6">
          <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
            Are you sure you want to remove this vehicle from your active listings? It will no longer be visible to renters.
          </p>
        </div>
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
          <button
            onClick={onClose}
            className="rounded-xl px-5 py-2.5 text-xs font-bold uppercase tracking-wider bg-slate-100 text-slate-600 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 transition-all cursor-pointer"
          >
            Cancel
          </button>
          <button
